#include <stdio.h>
#include <string.h>
#include "gbv.h"

int main(int argc, char *argv[]) {
    if (argc < 4) {
        printf("Uso: %s <opção> <biblioteca> <senha> [documentos...]\n", argv[0]);
        return 1;
    }

    const char *opcao = argv[1];
    const char *biblioteca = argv[2];
    const char *senha = argv[3];

     if(strcmp(opcao, "-c") == 0)
      gbv_create(biblioteca, senha);
       


    Library lib;
    if (gbv_open(&lib, biblioteca, senha) != 0) {
        printf("Erro ao abrir biblioteca %s\n", biblioteca);
        return 1;
    }

    if (strcmp(opcao, "-a") == 0) {
        for (int i = 4; i < argc; i++) {
            gbv_add(&lib, biblioteca, argv[i]);
        }
    } else if (strcmp(opcao, "-r") == 0) {
        for (int i = 4; i < argc; i++) {
            gbv_remove(&lib, argv[i]);
        }
    } else if (strcmp(opcao, "-l") == 0) {
        gbv_list(&lib);
    } else if (strcmp(opcao, "-v") == 0 && argc >= 5) {
        gbv_view(&lib, argv[4]);
    } else if (strcmp(opcao, "-o") == 0 && argc >= 5) {
        gbv_order(&lib, biblioteca, argv[4]);
    } else if (strcmp(opcao, "-n") == 0 && argc == 6) {
        gbv_rename(&lib, argv[4], argv[5]);
    } else if (strcmp(opcao, "-e") == 0 && argc == 6) {
        gbv_extract(&lib, argv[4], argv[5]);
    } else {
        printf("Opção inválida.\n");
    }

    return 0;
}